...
class CarDynamics(GenericKinematicsSE2):
    def __init__(self, parameters, c0, t0):
        ...
        #CarCommands.__init__(self, 0.0, 0.0)
        #CarParameters.__init__(self, 0.0)
    ...
    def integrate(self, dt, commands):
        ...
        linear_velocity = commands.linear_velocity
        steering_angle = commands.steering_angle
        wheel_distance = self.parameters.wheel_distance

        lateral = 0.0
        linear = [linear_velocity, lateral]

        angular = (linear_velocity*np.tan(steering_angle))/wheel_distance
