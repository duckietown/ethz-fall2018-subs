...
class CameraNode(object):
    ...
    def kmeans_algo(self, stream_data):
        ...
        centers = self.init_centers(Z,K)

        search = True
        while(search):
            assignments = self.assign_centers(centers,Z,K)
            centers_old = centers
            centers = self.update_centers(assignments,Z,K)
            changed = False
            for i in range(0,np.shape(centers)[0]):

                if(not np.array_equal(centers_old[i],centers[i])):
                    changed= True
                    break
    ...
    def update_centers(self,assignments,Z,K):
    ...
    def assign_centers(self,centers,Z,K):

        assignments= np.zeros((np.shape(Z)[0],1))
        for i in range(0,np.shape(Z)[0]):
            K_min=0
            min_dist = np.Inf
            for j in range(0,K):
                d0 = Z[i,0]-centers[j,0]
                d1 = Z[i,1]-centers[j,1]
                d2 = Z[i,2]-centers[j,2]
